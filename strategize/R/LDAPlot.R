plotResults_lda = function( hypotheticalTopicProportion = NULL,
                      n_fold = 3,Yobs,
                      topicProportions,
                      documents_list,
  ...
  if(!is.null(hypotheticalTopicProportion)){
    ...
    Qest_raw <- computeQ_lda_internal(pi   = hypotheticalTopicProportion,
                                      Yobs     = Yobs,
    ...
    SE_obs = exp(computeQse_lda(PI__ = hypotheticalTopicProportion,
...
if(is.null(hypotheticalTopicProportion)){
  ...
  #find mean topic proportion + set up delta method transformation
  {
    ...
    toSimplex_f = function(pi_){toSimplex(c(0,pi_))}
  ...
  log_pr_w <- computeQ_lda_internal(pi  = MeanTopicProportion,
                           Yobs     = Yobs,
                           pi_mat    = topicProportions,
                           doc_words = documents_list,
                           term_mat  = wordTopicDistributions,
  ...
  {
    PEN_VEC = NA
    minThis_max  = function(pi_,INDICES,PEN_VALUE,DOC_INDICES_U, D_INDICES_U){
      pi_ = toSimplex_f(pi_)
      rez_ = computeQ_lda_internal(pi  = pi_,
                          Yobs     = Yobs[INDICES],
  ...
  # Variance bound setup
  {
    ...
    logSE_meanPi <- computeQse_lda(PI__ = toSimplex_f(initVec_empiricalMean),
                                 ...
                                 LOG_TREATCOMBS=log_treatCombs,
                                 YOBS = Yobs,returnLog=T)
    logSE_flatPi <- computeQse_lda(PI__ = toSimplex_f(initVec_flat),
                                 INDICES_ = split1_indices,
                                 DOC_INDICES_U = doc_indices_u_split1,
                                 D_INDICES_U = d_indices_u_split1,
                                 TERMS_MAT_INPUT = wordTopicDistributions,
    ...
    # COBYLA algorithm
    {
      nloptr_sol <- optim_max_raw <- ((rsolnp_results <- nloptr::nloptr(x0 = initVec ,
                                                                        ...
                                                                        eval_g_ineq = function(pi_){
                                                                          pi__ = toSimplex_f(pi_)
                                                                          upperBound_variance_log <- computeQse_lda(PI__ = pi__,
                                                                                                                    INDICES_ = split1_indices,
                                                                                                                    DOC_INDICES_U = doc_indices_u_split1,
                                                                                                                    D_INDICES_U = d_indices_u_split1,
                                                                                                                    TERMS_MAT_INPUT = wordTopicDistributions,
                                                                                                                    ...
                                                                                                                    LOG_PR_W = log_pr_w,
                                                                                                                    YOBS = Yobs,returnLog=T)
                                                                          constrainThis_ <- c(upperBound_variance_log, pi__)
      ...
      optim_max_raw <- clip2((rsolnp_results <- Rsolnp::solnp(pars = initVec ,
                                                              ...
                                                              ineqfun = function(pi_){
                                                                ...
                                                                upperBound_variance_log <- computeQse_lda(PI__ = pi__,
                                                                                                          ...
                                                                                                          LOG_TREATCOMBS=log_treatCombs,
                                                                                                          YOBS = Yobs,returnLog=T)
                                                                return( c(upperBound_variance_log, pi__)  )  },
                                                              ineqLB = LB_VEC,  ineqUB = UB_VEC))$pars)
    }
    mySE = exp(computeQse_lda(PI__ = toSimplex_f(rsolnp_results$pars),
                              INDICES_ = split1_indices,
                              DOC_INDICES_U = doc_indices_u_split1,
                              D_INDICES_U = d_indices_u_split1,
                              TERMS_MAT_INPUT = wordTopicDistributions,
      ...
      function(pi){ with(data, {
        ...
        my_grad = ((c(rootSolve::gradient(f = function(x){
          ...
          logSE__ <- computeQse_lda(PI__ = pi__,
                                    ...
                                    LOG_TREATCOMBS=log_treatCombs,
                                    YOBS = Yobs,returnLog=T)
          constrainThese_ <- c(logSE__, pi__)
  ...
  Qest_raw = computeQ_lda_internal(pi   = optim_max,
  ...
  Qest_split = computeQ_lda_internal(pi   = optim_max,
  ...
  splitForSEObs <- "1"
  SE_obs = exp(computeQse_lda(PI__ = optim_max,
  ...
  splitForSEObs <- "1"
  SE_split = exp(computeQse_lda(PI__ = optim_max,
                                INDICES_ = eval(parse(text=sprintf("split%s_indices",splitForSEObs))),
                                DOC_INDICES_U = eval(parse(text=sprintf("doc_indices_u_split%s",splitForSEObs))),
                                D_INDICES_U = eval(parse(text=sprintf("d_indices_u_split%s",splitForSEObs))),
                                TERMS_MAT_INPUT = wordTopicDistributions,
  ...
  RETURN_LIST = list("PiStar"=c(optim_max),
                            "PiStar_se" = c(NA,optim_max_SEs_mEst),
                            "Q_point"=Qest_raw,
                            "Q_se" = SE_obs,
