...
strategize_register_jax_transformer_helpers <- function() {
  ...
  strategize_register_transformer_module()
  helper_code <- paste(
    "import jax",
    ...
    "    return x * inv_rms * g",
    "",
    "from strategize_transformer import self_attention as _strategize_self_attention",
    "",
...
strategize_register_jax_svi_helpers <- function() {
  ...
  helper_code <- paste(
    ...
    "",
    "def _strategize_svi_execute(fn, state, args, donate_state):",
    "    try:",
    "        result = fn(state, args)",
    "        return jax.block_until_ready(result) if donate_state else result",
    "    except Exception as exc:",
    "        if donate_state:",
    "            raise RuntimeError('Donated SVI update failed; its input state cannot be retried') from exc",
    "        raise",
    "",
    ...
    "    )",
    "",
    "def strategize_svi_update_jit(svi, svi_state, batch_args, forward_mode_differentiation=False, donate_state=False):",
    ...
    "    key = _strategize_svi_update_cache_key('single', svi, names, forward_mode_differentiation) + (bool(donate_state),)",
    ...
    "            )",
    "        fn = jax.jit(_compiled, donate_argnums=(0,) if donate_state else ())",
    "        _strategize_svi_update_jit_cache[key] = fn",
    "    return _strategize_svi_execute(fn, svi_state, args, donate_state)",
    "",
    "def strategize_svi_update_scan_jit(svi, svi_state, batch_args_chunks, forward_mode_differentiation=False, donate_state=False):",
    "    global _strategize_svi_update_jit_compile_count",
    "    names = _strategize_svi_update_arg_names(batch_args_chunks)",
    "    chunks = {name: batch_args_chunks[name] for name in names}",
    "    key = _strategize_svi_update_cache_key('scan', svi, names, forward_mode_differentiation) + (bool(donate_state),)",
    "    fn = _strategize_svi_update_jit_cache.get(key)",
    "    if fn is None:",
    "        _strategize_svi_update_jit_compile_count += 1",
    ...
    "        fn = jax.jit(_compiled, donate_argnums=(0,) if donate_state else ())",
    "        _strategize_svi_update_jit_cache[key] = fn",
    "    return _strategize_svi_execute(fn, svi_state, chunks, donate_state)",
    "",
