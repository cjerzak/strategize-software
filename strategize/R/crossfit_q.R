...
cs_crossfit_q_validate <- function(Y, W, pair_id, profile_order, p_list,
  ...
  if (isTRUE(adversarial)) {
    stop("crossfit_q currently supports non-adversarial average-case runs only.", call. = FALSE)
  }
  if (as.integer(K) != 1L) {
  ...
  list(pair_mat = pair_mat, pair_info = pair_info)
}
