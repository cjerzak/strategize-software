test_that("transformer MoE defaults resolve by inference method and depth", {
  ...
  expect_identical(cfg$transformer_moe$compute_dtype, "bfloat16")
  expect_true(cfg$transformer_moe$activation_checkpointing)
  expect_error(resolve(list(transformer_moe = list(compute_dtype = "fp8"))), "compute_dtype")
  expect_error(resolve(list(transformer_moe = list(activation_checkpointing = NA))), "activation_checkpointing")
