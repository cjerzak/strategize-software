...
#' @param use_memory_folding Enable DeepAgent-style memory folding
...
#' @param webpage Webpage reader configuration (use \code{webpage_options()})
...
asa_config <- function(backend = NULL,
  ...
  use_memory_folding <- use_memory_folding %||% ASA_DEFAULT_MEMORY_FOLDING
  ...
  if (!is.null(search) && !inherits(search, "asa_search")) {
    if (is.list(search)) {
      search <- tryCatch(
        do.call(search_options, search),
        error = function(e) {
          stop("`search` must be created with search_options() or be a list coercible to it",
               call. = FALSE)
        }
      )
    } else {
  ...
  # Validate webpage if provided
  if (!is.null(webpage) && !inherits(webpage, "asa_webpage")) {
    if (is.list(webpage)) {
      webpage <- tryCatch(
        do.call(webpage_options, webpage),
        error = function(e) {
          stop("`webpage` must be created with webpage_options() or be a list coercible to it",
               call. = FALSE)
        }
      )
    } else {
      stop("`webpage` must be created with webpage_options() or be a list",
           call. = FALSE)
    }
  }
...
print.asa_config <- function(x, ...) {
  ...
  cat("Memory Folding:  ", if (x$use_memory_folding) "Enabled" else "Disabled", "\n", sep = "")
  if (x$use_memory_folding) {
...
#' @param humanize_timing If TRUE, adds human-like jitter to delays (default: TRUE).
#' @param jitter_factor Jitter range as fraction of base delay (default: 0.5 = ±50%).
#' @param region DuckDuckGo region code (optional, best-effort).
#' @param safesearch Safe search mode (default: "moderate").
#' @param use_browser If TRUE, allows browser-based tiers (default: TRUE).
#' @param headless If TRUE, runs browser tiers headless (default: TRUE).
#'
...
search_options <- function(max_results = NULL,
  ...
  structure(
    list(
      ...
      captcha_backoff_base = captcha_backoff_base %||% 3.0,
      page_load_wait = page_load_wait %||% 2.0,
      inter_search_delay = inter_search_delay %||% ASA_DEFAULT_INTER_SEARCH_DELAY,
      humanize_timing = humanize_timing %||% TRUE,
      jitter_factor = jitter_factor %||% 0.5,
      region = region,
      safesearch = safesearch %||% "moderate",
      use_browser = use_browser %||% TRUE,
...
print.asa_search <- function(x, ...) {
  cat("Search Options: max_results=", x$max_results,
      ...
      ", browser=", if (isTRUE(x$use_browser)) "enabled" else "disabled",
      ", safesearch=", x$safesearch, "\n", sep = "")
  invisible(x)
}

#' Webpage Reader Options
#'
#' Configure the optional OpenWebpage tool for reading and extracting text from
#' public webpages. This capability is disabled by default and must be enabled
#' per call (e.g., via \code{run_task(webpage = webpage_options(allow_read_webpages = TRUE))})
#' or by placing a \code{webpage} object in \code{asa_config()}.
#'
#' @param allow_read_webpages If TRUE, allows the agent to open and read full webpages.
#' @param relevance_mode Relevance selection for excerpts. One of: "auto", "lexical", "embeddings".
#' @param embedding_provider Embedding provider. One of: "auto", "openai", "sentence_transformers".
#' @param embedding_model Embedding model identifier.
#' @param embedding_api_base Optional OpenAI-compatible embedding base URL.
#' @param timeout Request timeout in seconds for fetching webpages.
#' @param max_bytes Maximum bytes to download per page.
#' @param max_chars Maximum characters of extracted text to return.
#' @param max_chunks Maximum chunks to return.
#' @param chunk_chars Chunk size for splitting extracted text.
#' @param prefilter_k Pre-filter candidate chunk count before relevance selection.
#' @param use_mmr If TRUE, use MMR for diverse excerpt selection.
#' @param mmr_lambda MMR trade-off parameter (0..1).
#' @param cache_enabled Enable in-memory cache.
#' @param cache_max_entries Maximum cached pages.
#' @param cache_max_text_chars Maximum cached text.
#' @param user_agent User-Agent string for requests.
#'
#' @return An object of class \code{asa_webpage}
#'
#' @export
webpage_options <- function(allow_read_webpages = FALSE,
                            relevance_mode = "auto",
                            ...
                            cache_max_text_chars = 200000L,
                            user_agent = "ASA-Research-Agent/1.0") {
  structure(
    list(
      ...
      use_mmr = isTRUE(use_mmr),
      mmr_lambda = as.numeric(mmr_lambda),
      cache_enabled = isTRUE(cache_enabled),
      cache_max_entries = as.integer(cache_max_entries),
      cache_max_text_chars = as.integer(cache_max_text_chars),
...
#' Print Method for asa_webpage Objects
#'
#' @param x An asa_webpage object
#' @param ... Additional arguments (ignored)
#'
#' @method print asa_webpage
#' @export
print.asa_webpage <- function(x, ...) {
  cat(
    "Webpage Reader: allow_read_webpages=",
    if (isTRUE(x$allow_read_webpages)) "TRUE" else "FALSE",
    ", relevance_mode=", x$relevance_mode,
    ", embedding_provider=", x$embedding_provider,
