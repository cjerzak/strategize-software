computeQ_lda_internal <- function(theta=NULL,
                         term_mat,
                         Yobs,
                         doc_words,
                         dtm=NULL,
  ...
  for(ijack in 1:n_iters){
    ...
    if(is.null(TreatFxn)){
      theta_mat = replicate(theta,n=ndocs)
    }
    if(!is.null(TreatFxn)){
      theta_mat = TreatFxn(alpha_mat,theta)
    }
    ...
    {
      {
        Q_wts_noTrim_uniformDenom <- length(log_pr_w_given_theta)*exp(log_pr_w_given_theta)/sum(exp(log_pr_w_given_theta))
        Q_wts_uniformDenom <- length(log_pr_w_given_theta)*exp(trim_fxn(log_pr_w_given_theta-matrixStats::logSumExp(log_pr_w_given_theta),trim_q))
      ...
      {
        Q_wts_noTrim <- exp(log_pr_w_given_theta-log_pr_w)
        if(!smoothWts){Q_wts <- trim_fxn(exp(log_pr_w_given_theta-log_pr_w),  trim_q)}
        if(smoothWts){Q_wts <- trim_fxn(SRatFxn(log_pr_w_given_theta ,  log_pr_w ,100),trim_q)}
      }
    ...
    if(!is.null(dtm)){
      ...
      #estimate Y(theta1) using estimated pi_i's as linear predictors (preferred)
      {
        lm_dat_est = cbind(Yobs,t(pi_mat))
        colnames(lm_dat_est)[-1] <- 1:nrow(pi_mat)
        my_lm_est = lm(Yobs~.,as.data.frame(lm_dat_est))
        newdata1_est = t(lm_dat_est[1,]);newdata1_est[-1] <- theta1
