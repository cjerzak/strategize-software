computeQse_lda = function(PI__,INDICES_, DOC_INDICES_U, D_INDICES_U,
                            PI_MAT_INPUT,MARGINAL_BOUNDS,DOC_LIST,
                            MODAL_DOC_LEN,hypotheticalN = NULL,
                            TERMS_MAT_INPUT,LOG_TREATCOMBS,YOBS,
                            returnLog = T,LOG_PR_W=NULL){

    # Set up greedy algorithm for computing log_maxProb
    logW = log(apply( term_mat * c(PI__),2,sum))
    logW = logW[names(MARGINAL_BOUNDS)];
    logW_counter = logW; logW_counter[] <- 0
    my_tmp = rep(NA,times=MODAL_DOC_LEN);log_maxProb <- 0;for(fa in 1:MODAL_DOC_LEN){
      which_max = which.max(logW[logW_counter < MARGINAL_BOUNDS])
      my_tmp[fa] <- names(which_max)
      log_maxProb <- log_maxProb + logW[names(which_max)]
      logW_counter[names(which_max)] <- logW_counter[names(which_max)] + 1
    }  #check that marginal bounds respected: table(my_tmp); MARGINAL_BOUNDS[names(table(my_tmp))]

    # Perform weighting to obtain scaling factor
    PI_MAT_ = PI_MAT_INPUT; PI_MAT_[] <- PI__
    NUM__ = logPrWGivenPi_fxn(doc_indices     = DOC_LIST[INDICES_],
                           pi_mat = PI_MAT_[,INDICES_],
                           terms_posterior = TERMS_MAT_INPUT,
                           doc_indices_u = DOC_INDICES_U,
                           d_indices_u = D_INDICES_U)
    if(!is.null(LOG_PR_W)){ LOG_PR_W <- LOG_PR_W[INDICES_] }
    if(is.null(LOG_PR_W)){
      LOG_PR_W <- logPrWGivenPi_fxn(doc_indices     = DOC_LIST[INDICES_],
                             pi_mat = PI_MAT_INPUT[,INDICES_],
                             terms_posterior = TERMS_MAT_INPUT,
                             doc_indices_u = DOC_INDICES_U,
                             d_indices_u = D_INDICES_U)
    }
    scaleFactor = sum(YOBS[INDICES_]^2 * prop.table(exp(NUM__ - LOG_PR_W  ))  )

    # Combine terms to get overall variance - log scale for improved numerical stability
    logN <- log(length(INDICES_))
    if(!is.null(hypotheticalN)){logN <- log(hypotheticalN)}
    upperBound_se_VE_log = (log(scaleFactor) + LOG_TREATCOMBS + log_maxProb - logN)
    upperBound_se_EV_log = (log(var(YOBS[INDICES_])) + LOG_TREATCOMBS + log_maxProb - logN)
    upperBound_se_ <- 0.5*matrixStats::logSumExp(c(upperBound_se_EV_log,upperBound_se_VE_log))#0.5 for sqrt
    if(returnLog == F){upperBound_se_ <- exp(upperBound_se_) }
    return( upperBound_se_ )
  }

